...
## History
##
## 2010-12-13 : * Added the contrasts that were developed for the
##            :   'normal' analysis, so we have both pre- and post-
##            :   response regressors.
##
## 2010-07-15 : * Added regressors for jitter (post-response)
##            :   periods.
##
## 2010-06-29 : * Added more contrasts for version 4.
##
...
CONTRAST_LIST   = ('IP', 'IN', 'XP', 'XN', 'Probe',
                   'pIP', 'pIN', 'pXP', 'pXN',
                   'I', 'X', 'I > X', 'X > I',
                   'pI', 'pX', 'pI > pX', 'pX > pI',
                   'N', 'P', 'N > P', 'P > N',
                   'pN', 'pP', 'pN > pP', 'pP > pN',
                   'IN > IP', 'IP > IN', 'XN > XP', 'XP > XN', 
                   'IN > XN', 'IP > XP', 'XN > IN', 'XP > IP',
                   'pIN > pIP', 'pIP > pIN', 'pXN > pXP', 'pXP > pXN', 
                   'pIN > pXN', 'pIP > pXP', 'pXN > pIN', 'pXP > pIP',
                   'd(IP)', 'd(IN)', 'd(XP)', 'd(XN)',
                    'XN+IP > IN+XP', '2XN > IN+XP', 'IN+XN > 2XP',
                   )

CONTRASTS       = {
    'IP' : [1, 0, 0, 0, 0, 0, 0, 0, 0],
    'IN' : [0, 1, 0, 0, 0, 0, 0, 0, 0],
    'XP' : [0, 0, 1, 0, 0, 0, 0, 0, 0],
    'XN' : [0, 0, 0, 1, 0, 0, 0, 0, 0],
    'Probe'  : [0, 0, 0, 0, 1, 0, 0, 0, 0],

    'pIP' : [0, 0, 0, 0, 0, 1, 0, 0, 0],
    'pIN' : [0, 0, 0, 0, 0, 0, 1, 0, 0],
    'pXP' : [0, 0, 0, 0, 0, 0, 0, 1, 0],
    'pXN' : [0, 0, 0, 0, 0, 0, 0, 0, 1],
        
    'I' : [0.5, 0.5, 0, 0, 0,  0, 0, 0, 0],
    'X' : [0, 0, 0.5, 0.5, 0,  0, 0, 0, 0],
    
    'I > X' : [0.5, 0.5, -0.5, -0.5, 0,  0, 0, 0, 0],
    'X > I' : [-0.5, -0.5, 0.5, 0.5, 0,  0, 0, 0, 0],
    
    'pI' : [0, 0, 0, 0, 0,  0.5, 0.5, 0, 0],
    'pX' : [0, 0, 0, 0, 0,  0, 0, 0.5, 0.5],

    'pI > pX' : [0, 0, 0, 0, 0,  0.5, 0.5, -0.5, -0.50],
    'pX > pI' : [0, 0, 0, 0, 0,  -0.5, -0.5, 0.5, 0.5],

    'N' : [0, 0.5, 0, 0.5, 0,  0, 0, 0, 0],
    'P' : [0.5, 0, 0.5, 0, 0,  0, 0, 0, 0],

    'N > P' : [-0.5, 0.5, -0.5, 0.5, 0,  0, 0, 0, 0],
    'P > N' : [0.5, -0.5, 0.5, -0.5, 0,  0, 0, 0, 0],

    'pN' : [0, 0, 0, 0, 0, 0, 0.5, 0, 0.5],
    'pP' : [0, 0, 0, 0, 0, 0.5, 0, 0.5, 0],

    'pN > pP' : [0, 0, 0, 0, 0, -0.5, 0.5, -0.5, 0.5],
    'pP > pN' : [0, 0, 0, 0, 0, 0.5, -0.5, 0.5, -0.5],


    'IN > IP' : [-1, 1, 0, 0, 0,  0, 0, 0, 0],
    'IP > IN' : [1, -1, 0, 0, 0,  0, 0, 0, 0],
    'XN > XP' : [0, 0, -1, 1, 0,  0, 0, 0, 0],
    'XP > XN' : [0, 0, 1, -1, 0,  0, 0, 0, 0],
    
    'IN > XN' : [0, 1, 0, -1, 0, 0, 0, 0, 0],
    'IP > XP' : [1, 0, -1, 0, 0, 0, 0, 0, 0],
    'XN > IN' : [0, -1, 0, 1, 0, 0, 0, 0, 0],
    'XP > IP' : [-1, 0, 1, 0, 0, 0, 0, 0, 0],

    'pIN > pIP' : [0,  0, 0, 0, 0, -1, 1, 0, 0],
    'pIP > pIN' : [0,  0, 0, 0, 0, 1, -1, 0, 0],
    'pXN > pXP' : [0,  0, 0, 0, 0, 0, 0, -1, 1],
    'pXP > pXN' : [0,  0, 0, 0, 0, 0, 0, 1, -1],
    
    'pIN > pXN' : [0, 0, 0, 0, 0, 0, 1, 0, -1],
    'pIP > pXP' : [0, 0, 0, 0, 0, 1, 0, -1, 0],
    'pXN > pIN' : [0, 0, 0, 0, 0, 0, -1, 0, 1],
    'pXP > pIP' : [0, 0, 0, 0, 0, -1, 0, 1, 0],


    'd(IP)' : [1, -0.3333, -0.3333, -0.3333, 0, 0, 0, 0, 0],
    'd(IN)' : [-0.3333, 1, -0.3333, -0.3333, 0, 0, 0, 0, 0],
    'd(XP)' : [-0.3333, -0.3333, 1, -0.3333, 0, 0, 0, 0, 0],
    'd(XN)' : [-0.3333, -0.3333, -0.3333, 1, 0, 0, 0, 0, 0],

    'XN+IP > IN+XP' : [0.5, -0.5, -0.5, 0.5, 0, 0, 0, 0, 0],
    '2XN > IN+XP' : [0, -0.5, -0.5, 1, 0, 0, 0, 0, 0],
    'IN+XN > 2XP' : [0, 0.5, -1, 0.5, 0, 0, 0, 0, 0],
}
...
def Parse(filename):
    ...
    for block in range(1,len(FIXATIONS)+1):
        ...
        for practice in ['+', '-']:
            ...
            ## Write trial onsets (in secs) 
            obs.write("SESSION %d / ENCODING / P%s \t %d\n" % (block, practice, len(selected)))
            conds.write("POST_ENCODING / P%s / Onsets\t" % (practice))
            mfile.write("names{%d}='POST_ENC/P%s';\n" % (COND, practice))
            mfile.write("onsets{%d}=[" % COND)
            for x in selected:
                t = (x.RelativeTime(x.Encoding)+x.EncodingRT)/1000.0
                conds.write("%1.1f " % round(t,1))
                mfile.write("%1.1f " % round(t,1))
            conds.write("\n")
            mfile.write("];\n")
            ...
            for x in selected:
                t = (x.RelativeTime(x.Encoding)+x.EncodingRT)/1000.0
                d = (x.RelativeTime(x.Execution)/1000.0) - round(t,1)
                conds.write("%1.1f " % round(d,1))
                mfile.write("%1.1f " % round(d,1))
            conds.write("\n")
            mfile.write("];\n")
        ...
        for practice in ['+', '-']:
            selected   = [x for x in subset if x.Practiced == practice]
            ## Write trial onsets (in secs) 
            obs.write("SESSION %d / PSOT_EXECUTION / P%s \t %d\n" % (block, practice, len(selected)))
            conds.write("POST_EXECUTION / P%s / Onsets\t" % (practice))
            mfile.write("names{%d}='POST_EXE/P%s';\n" % (COND, practice))
            mfile.write("onsets{%d}=[" % COND)
            for x in selected:
                t = (x.RelativeTime(x.Execution)+x.ExecutionRT)/1000.0
                conds.write("%1.1f " % round(t,1))
                mfile.write("%1.1f " % round(t,1))
            conds.write("\n")
            mfile.write("];\n")
            ...
            for x in selected:
                t = (x.RelativeTime(x.Execution)+x.ExecutionRT)/1000.0
                d = (x.RelativeTime(x.Probe)/1000.0) - round(t,1)
                conds.write("%1.1f " % round(d,1))
                mfile.write("%1.1f " % round(d,1))
            conds.write("\n")
            mfile.write("];\n")
