...
## History
##
## 2010-06-29 : * Added more contrasts foir version 4.
##
...
def Parse(filename):
    ...
    for block in range(1,len(FIXATIONS)+1):
        ...
        valid    = [x for x in correct if x.EncodingRT <= 20000
                    and x.ExecutionRT <= 200000]
        excess   = [x for x in correct if x.EncodingRT > 20000
                    or x.ExecutionRT > 200000]
        ...
        discard  = errors+outliers+excess

        print "Block %d: Errors %d, Outliers %d, Excess %d" % (block, len(errors), len(outliers), len(excess))
