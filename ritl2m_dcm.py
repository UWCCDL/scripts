...
class Trial:
    ...
    def ScanTime(self, val):
        return self.RelativeTime(val) + self.blockOffset
...
def Parse(filename, blockLengths):
    ...
    FIRST_TRIALS = []
    previous = None
    blockLengths=[0]+blockLengths
    for t in trials:

        if  t.block > len(blockLengths):
            raise Exception, "Not enough block lengths supplied"

        t.blockOffset = 2 * reduce(add, blockLengths[0:t.block])
        #print t.block, blockLengths[0:t.block]
        
        # print t.blockOffset
        # Identify the first trials
    ...
    # For DCM analysis, we need a special model that contains
    # All the trials in a particular format, and that does
    # not distinguish between the different shoes.

    correct = [t for t in trials if t.acc == 1]
    errors  = [t for t in trials if t.acc == 0]
    ...
    for phase in ['Encoding', 'Execution']: 
        for practice in [['Yes', 'No'], ['No']]:
            appropriate = [c for c in correct if c.practiced in practice]
            pName = ""
            if len(practice) == 1:
                pName = "_Novel"
            fout.write("names{%d}='%s%s';\n" % (i, phase, pName))
            onsets = "%s" % [a.ScanTime(a.onsets[phase]) for a in appropriate]
    ...
    onsets = "%s" % [c.ScanTime(c.onsets['Probe']) for c in correct]
    ...
    if len(errors) > 0:
        for phase in ['Encoding', 'Execution', 'Probe']:
            O = [e.ScanTime(e.onsets[phase]) for e in errors if e.onsets[phase]>0 and e.rts[phase]>0]
    ...
    #print blockLengths
    rout = open("s%s_session_regressors.txt" % subject, 'w')
    for i in range(len(blockLengths[1:])):
        for j in range(blockLengths[i+1]):
            for s in range(len(blockLengths[1:])-1):  # Do not print a column for last block
                if s == i:
                    rout.write("1 ")
                else:
                    rout.write("0 ")
